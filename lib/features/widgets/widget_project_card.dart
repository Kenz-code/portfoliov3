...
class _WidgetProjectCardState extends State<WidgetProjectCard>
  ...
  void initState() {
    ...
    _scaleAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(
      CurvedAnimation(parent: _entranceController, curve: Curves.easeOutCirc),
    );
  }
  ...
  void _onVisibilityChanged(VisibilityInfo info) {
    if (!_hasAppeared && info.visibleFraction > 0.1) {
      _hasAppeared = true;
  ...
  Widget build(BuildContext context) {
    return VisibilityDetector(
      key: Key('project-card-${widget.projectUtils.title}'),
                              ...
                              return ColorFiltered(
                                colorFilter: ColorFilter.mode(
                                  Colors.black.withValues(
                                      alpha: _imageDarkenAnimation.value),
                                  BlendMode.darken,
                                ),
                                child: Image.asset(
